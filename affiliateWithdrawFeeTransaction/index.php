...
<?php ob_start(); ?>
	<table width="100%">
		<tr>
			<td align="left"><h1>KOMISI AFFILIATE BELUM DI BAYAR</h1></td>
			<td align="right">
				<input type="button" value="KEMBALI" style="width:200px" onclick='window.location="../affiliateWithdrawFee/index.php"' />
			</td>
		</tr>	
	</table>	
			...
			<table width="100%" cellpadding="7">
				...
				<tr>
					<td valign="top" colspan="2">
						...
						<select name="affiliateId" style="width:100%;"  >
							<?php while($val = mysqli_fetch_array($cmbAffiliate)): ?>
								<option value="<?php echo $val['id'] ?>" <?php echo $affiliateId == $val['id'] ? 'selected' : '' ?>><?php echo strtoupper($val['name']) ?></option>	
							<?php endwhile; ?>	
						</select>				
					</td>													
				</tr>	
	...
	<?php else: ?>	
		...
		<form action="addSave.php" method="post" id="transfetCommision">
			...
			<div id="tbl">
				<table width="100%" id="tblaffiliate">
					<thead>			
						<tr>
							...
							<th style="font-size:12px" align="center" width="%">KOMISI AFFILIATE</th>													
						</tr>	
					</thead>
					<tbody>
						<?php $i=1; ?>
						...
						<?php while($val = mysqli_fetch_array($data)): ?>
							<tr style="cursor:pointer">
								<td align="center">
									<input style="cursor:pointer" name="salesOrderId[]" class="chkSalesOrderId" type="checkbox" value="<?php echo $val['id']  ?>-<?php echo $val['no_order']  ?>-<?php echo $val['amount_fee_affiliate'] ?>" />																			
								</td>
								...
								<td align="center"><?php echo number_format($val['amount_fee_affiliate'],0,'','.') ?></td>
							</tr>																
							<?php $totalNilaiJual = $totalNilaiJual + $val['total_nilai'] ?>
							<?php $totalKomisi = $totalKomisi + $val['amount_fee_affiliate'] ?>
							<?php $i++; ?>
						<?php endwhile; ?>
					</tbody>
					<tfoot>
			...
			<fieldset style="margin-top: 15px">
			   <div id="modalPaymentx" style="margin:2px" style="display: block;">
				...
				<table width="100%" >
					<tr>
						<td width="25%">NAMA AFFIILIATE</td>
						<td><b><?php echo strtoupper($dataAffiliate['name']) ?></td>
					</tr>	
					<tr>
						<td>TANGGAL TRANSFER</td>
						<td><input type="text" name="affiliateDateTransfer" id="affiliateDateTransfer" value="" readonly style="width:100%" /></td>
					</tr>	
					<tr>
						<td>TUJUAN TRANSFER</td>
						<td>
							<select name="affiliateBankTo" style="width:100%;"  >
								<?php while($val = mysqli_fetch_array($cmbAffiliateBank)): ?>
					...
					<tr>
						<td>JUMLAH KOMISI</td>
						<td>
						  <input type="hidden" name="affiliateTotalTransfer" id="affiliateTotalTransfer" value="0">	
						  <b><div id="affiliateTotalTransferLabel">0</div></b>
						 </td>
					</tr>	
					<tr>
	...
	$(document).ready(function() {
		...
		$(function() {
				$( "#affiliateDateTransfer" ).datepicker({
					dateFormat : 'dd/mm/yy',
					changeMonth : true,
					changeYear : true,
					yearRange: '-100y:c+nn',
				...
				<?php $tmp = strlen(trim($_REQUEST['affiliateDateTransfer'])) == 0 ?  '' : explode('/',$_REQUEST['affiliateDateTransfer']) ?>
				$("#affiliateDateTransfer" ).datepicker("setDate", <?php if(is_array($tmp)) : ?> new Date(<?php echo ($tmp[2]) ?>,<?php echo ($tmp[1]-1) ?>,<?php echo $tmp[0] ?>) <?php else: ?> null <?php endif; ?>);
			});


	    $(".chkSalesOrderId").click(function () {
            //Fetch all row CheckBoxes in the Table.
            var chkRows = $("#tblaffiliate").find(".chkSalesOrderId");
            var total = 0;	
			...
			$("#affiliateTotalTransferLabel").html(total);
			$("#affiliateTotalTransferLabel").simpleMoneyFormat();
			$("#affiliateTotalTransfer").val(total);
