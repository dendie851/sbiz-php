<?php 
	...
	$query = "select so.id, so.name, so.address_shipping, so.phone, so.no_order,is_reseller, 
		  ...
		  (select w.name from warehouse_external as w where w.id = warehouse_external_id) as warehouse_external_name,								  		  (select m.name from member as m where m.id = so.sales_id) as sales_name,
