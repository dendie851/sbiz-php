<?php 
	...
	$query = "select s.id, s.no_order, s.client_id, s.period_order_id, s.name, s.address_shipping, s.tipe_order, packing_option, sd.is_bundling,
			...
			s.districts, s.districts_sub, s.postal_code, sd.amount as stuff_amount, sd.id as sales_order_detail_id
	...
	while($val = mysqli_fetch_array($data)) {
		...
		if($val['is_bundling'] == '0') {	
		  ...
		  $row++;        
	       ...
	       $query = "select b.id, b.stuff_id, b.qty, s.name, s.sku
				 from sales_order_detail_bundling as b
				 inner join stuff as s
				   on s.id = b.stuff_id
				 where sales_order_detail_id = '{$val['sales_order_detail_id']}'
				 order by id asc";
		  $rstDetailBundling = mysqli_query($con, $query) or die (mysqli_error($con));

		  while($dataDetailBundling = mysqli_fetch_array($rstDetailBundling)) {
			  $objPHPExcel->getSheet(0)
		        ->setCellValue('A'.$row, $val['no_order'])
		        ->setCellValue('B'.$row, strtolower($val['sales_name']))
		        ->setCellValue('C'.$row, strtolower($salesPhoneNew))
		        ...
		        ->setCellValue('I'.$row, strtolower($val['postal_code']))
		        ->setCellValue('J'.$row, strtoupper($val['warehouse_external_code']))	        
		        ->setCellValue('K'.$row, strtoupper($dataDetailBundling['sku']))
		        ->setCellValue('L'.$row, strtolower($dataDetailBundling['qty'] * $val['stuff_amount']))
		        ->setCellValue('M'.$row, strtoupper($serviceOptionLabel))
		        ->setCellValue('N'.$row, strtoupper($packingOptionLabel));
		  		$row++;        
		  }						        
