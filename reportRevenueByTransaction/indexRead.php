<?php 
	...
	$finSourceFundId = array(); 
	if(isset($_REQUEST['finSourceFundId'])) {
	  $finSourceFundId = $_REQUEST['finSourceFundId'];
	} else {
	  if(isset($_REQUEST['finSourceFundIdChoose'])) {
	  	$finSourceFundId = explode(',',$_REQUEST['finSourceFundIdChoose']); 
	  }
	}
	...
	if(count($finSourceFundId) > 0) {
		$finSourceFundIdChoose = '';
		foreach($finSourceFundId as $val) {
			$finSourceFundIdChoose = $finSourceFundIdChoose.$val.',';
		}
		$finSourceFundIdChoose = substr($finSourceFundIdChoose,0,strlen($finSourceFundIdChoose)-1);
	} else { 
		if(strlen($_REQUEST['finSourceFundIdChoose']) > 0) {
			$finSourceFundIdChoose = $_REQUEST['finSourceFundIdChoose'] ;
		} else {
			$query = "select id,name 
	...
	if(isset($_REQUEST['buyer'])) {
		...
		$query = "select so.id, so.no_order, so.client_id, so.period_order_id, so.name, so.address_shipping, so.tipe_order,
				...
				( ((sod.price * sod.amount) - (((sod.price * sod.amount) / 100) * (so.discount_persen ))  - (so.discount_amount / (select count(sod_count.id) from sales_order_detail as sod_count where sod_count.sales_order_id = so.id)) ) - ((sod.price_basic * sod.amount) + ((sod.price_basic * sod.amount) * (so.platform_market_fee_percent/100)  )) ) as jml_laba_per_category,
				( (so.amount_sale - ((so.amount_sale / 100) * so.discount_persen)) + shipping_cost) as total,
				so.phone, so.platform_market_fee_percent
		...
		$query = "select so.id, so.no_order, so.client_id, so.period_order_id, so.name, so.address_shipping, so.tipe_order,
			...
			*/
			...
			$query = "select so.id, so.no_order, so.client_id, so.period_order_id, so.name, so.address_shipping, so.tipe_order,
				...
				date_format(so.date_payment,'%d/%m/%Y') as date_payment_frm,
				date_format(so.date_packing,'%d/%m/%Y') as date_packing_frm,
				(so.discount_amount / (select count(sod_count.id) from sales_order_detail as sod_count where sod_count.sales_order_id = so.id)) as discount_amount_div,
				date_format(so.date_shipping,'%d/%m/%Y') as date_shipping_frm, no_resi,
				( ((so.amount_sale - ((so.amount_sale / 100) * so.discount_persen))  - discount_amount) + so.shipping_cost) as jml,
				( (so.amount_sale / 100) * so.discount_persen) as jml_diskon,
				( ((so.amount_sale - ((so.amount_sale / 100) * so.discount_persen)) - discount_amount) - so.amount_basic_sale) as jml_laba,
				(select m.name from member as m where m.id = sales_id) as sales_name,
				(select c.name from stuff_category as c where c.id = s.category_id) as category_name,
				(sod.price) as price, (sod.price_basic) as price_basic, sod.amount as sod_amount,
				(((sod.price * sod.amount) / 100) * (so.discount_persen )) as jml_diskon_per_category,
				(so.shipping_cost / (select count(sod_item.id) as jml_item 
				                   from sales_order_detail as sod_item where sod_item.sales_order_id = so.id)) 
				                   as shipping_cost_per_category,
			...
			*/
			...
			$query = "select so.id, so.no_order, so.client_id, so.period_order_id, so.name, so.address_shipping, so.tipe_order,
				...
				(sod.price) as price, (sod.price_basic) as price_basic, sod.amount as sod_amount,
				(((sod.price * sod.amount) / 100) * (so.discount_persen )) as jml_diskon_per_category,
				(so.shipping_cost / (select count(sod_item.id) as jml_item from sales_order_detail as sod_item where sod_item.sales_order_id = so.id)) as shipping_cost_per_category,
				( ((sod.price * sod.amount) - (((sod.price * sod.amount) / 100) * (so.discount_persen ))  - (so.discount_amount / (select count(sod_count.id) from sales_order_detail as sod_count where sod_count.sales_order_id = so.id)) ) - ((sod.price_basic * sod.amount) + ((sod.price_basic * sod.amount) * (so.platform_market_fee_percent/100)  )) ) as jml_laba_per_category,
				(select f.name from fin_source_fund as f where f.id = fin_source_fund_id) as fin_source_fund_name,
				so.platform_market_fee_percent							
			from sales_order as so
			inner join sales_order_detail as sod 
			  on sod.sales_order_id = so.id
			inner join stuff as s
