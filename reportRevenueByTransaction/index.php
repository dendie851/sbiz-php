		...
		<?php else: ?>
			<div style="margin: 10px 0px 10px 0px">
				<div style="text-align:left; float: left">
					<input type="button" value="PRINT DATA PEMBELI" onclick="window.open('printBuyer.php?categoryIdChoose=<?php echo $categoryIdChoose ?>&finSourceFundIdChoose=<?php echo $finSourceFundIdChoose ?>&buyer=1&statusOrder=<?php echo $statusOrder ?>&statusPayment=<?php echo $statusPayment ?>&dateFrom=<?php echo urlencode($_REQUEST['dateFrom']) ?>&dateTo=<?php echo urlencode($_REQUEST['dateTo']) ?>')" />
					<input type="button" value="EXPORT DATA PEMBELI KE EXCEL" onclick="window.open('excelBuyer.php?categoryIdChoose=<?php echo $categoryIdChoose ?>&finSourceFundIdChoose=<?php echo $finSourceFundIdChoose ?>&buyer=1&statusOrder=<?php echo $statusOrder ?>&statusPayment=<?php echo $statusPayment ?>&dateFrom=<?php echo urlencode($_REQUEST['dateFrom']) ?>&dateTo=<?php echo urlencode($_REQUEST['dateTo']) ?>')" />
				</div>	

				<div style="text-align:right">
					<input type="button" value="PRINT PENJUALAN" onclick="window.open('print.php?costAdds=<?php echo $costAdds ?>&categoryIdChoose=<?php echo $categoryIdChoose ?>&finSourceFundIdChoose=<?php echo $finSourceFundIdChoose ?>&statusOrder=<?php echo $statusOrder ?>&statusPayment=<?php echo $statusPayment ?>&dateFrom=<?php echo urlencode($_REQUEST['dateFrom']) ?>&dateTo=<?php echo urlencode($_REQUEST['dateTo']) ?>')" />
					...
					<tbody>
						...
						<?php while($val = mysql_fetch_array($data)): ?>
							<tr>
								...
								<td align="center">
									<?php if(!in_array(count($categoryId), array(0,$jmlDataCategory))): ?>
									   ...
									   <?php echo number_format(($amount_basic_sale ),0,'','.') ?>	
								...
								<td align="center">
									<?php if(!in_array(count($categoryId), array(0,$jmlDataCategory))): ?>
										<?php echo number_format($val['jml_diskon_per_category'],0,'','.')?>
								...
								<td align="center">
									<?php if(!in_array(count($categoryId), array(0,$jmlDataCategory))): ?>
										<?php //echo number_format($val['discount_amount'],0,'','.')?>
										<?php //echo number_format($val['discount_amount'],0,'','.')?><br /> 
										<?php echo number_format($val['discount_amount_div'],0,'','.')?><br /> 
									<?php else: ?>	
										<?php echo number_format($val['discount_amount'],0,'','.')?><br />
									<?php endif; ?>	
								</td>	
							...
							<?php if(!in_array(count($categoryId), array(0,$jmlDataCategory))): ?> 
								...
								<?php //$totalHargaDasar = $totalHargaDasar + ($val['price']  * $val['sod_amount'])'' ?>								
