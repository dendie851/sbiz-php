		...
		<form action="index.php" method="get">
			<table width="100%" cellpadding="7">
				<tr>
					...
					<td width="25%" valign="top">
						STATUS BAYAR KOMISI<br />
						<select name="statusCommission" style="width:100%">
							<option value="x" <?php echo 'x' == (isset($_REQUEST['statusCommission']) ? $_REQUEST['statusCommission'] :$statusCommission) ? 'selected' : '' ?>>Semua</option>							
							<option value="1" <?php echo '1' == (isset($_REQUEST['statusCommission']) ? $_REQUEST['statusCommission'] : $statusCommission) ? 'selected' : '' ?>>Sudah di Bayar</option>
							<option value="0" <?php echo '0' == (isset($_REQUEST['statusCommission']) ? $_REQUEST['statusCommission'] : $statusCommission) ? 'selected' : '' ?>>Belum di Bayar</option>
						</select>				
					</td>								
				...
				<table width="100%">
					<thead>			
						<tr>
							...
							<th style="font-size:12px" align="center" width="%">NAMA</th>						
							<th style="font-size:12px" align="center" width="%">JUMLAH TRANSAKSI</th>
							<th style="font-size:12px" align="center" width="%" style="font-size: 11px">JUMLAH NILAI TRANSAKSI <br />UNTUK RESELLER</th>
							<th style="font-size:12px" align="center" width="%">KOMISI RESELLER</th>													
							<th style="font-size:12px" align="center" width="%">POIN RESELLER</th>													
					...
					<tbody>
						...
						<?php while($val = mysql_fetch_array($data)): ?>
							<tr>
								...
								<td align="center"><?php echo number_format($val['total_fee_reseller'],0,'','.') ?></td>							
								<td align="center"><?php echo number_format($val['total_poin'],0,'','.') ?></td>							
								<td align="center">
									<span class="button"> 
										<a data-title="DETAIL" data-width="650" data-height="400" href="detail.php?resellerId=<?php echo $val['reseller_id'] ?>&statusOrder=<?php echo $statusOrder ?>&statusPayment=<?php echo $statusPayment ?>&dateFrom=<?php echo urlencode($_REQUEST['dateFrom']) ?>&dateTo=<?php echo urlencode($_REQUEST['dateTo']) ?>&statusCommission=<?php echo $statusCommission ?>" />[ DETAIL ]</a>
									<span>		
								</td>	
							...
							<?php $totalPoin = $totalKomisi + $val['total_poin'] ?>
							<?php $i++; ?>
						<?php endwhile; ?>
					<tbody>
					<tfoot>
						<th colspan="2">TOTAL</th>
						<th><?php echo number_format($totalTransaksi,0,'','.') ?> Transaksi</th>
						<th><?php echo number_format($totalNilaiJual,0,'','.') ?></th>
						<th><?php echo number_format($totalKomisi,0,'','.') ?></th>
						<th><?php echo number_format($totalPoin,0,'','.') ?></th>
						<th>&nbsp;</th>
					</tfoot>	
				</table>			
			</div>
