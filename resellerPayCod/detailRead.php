<?php 
	...
	$query = "select w.id,w.sales_order_id,w.sales_order_number,w.amount_cod_reseller,
	       so.name, so.date_order, so.no_order,
	       date_format(so.date_order,'%d %M %Y') as date_order_frm
		from reseller_withdraw_cod_detail as w
		inner join sales_order as so
		on so.id = w.sales_order_id 
		where w.reseller_withdraw_cod_id = '$id'
