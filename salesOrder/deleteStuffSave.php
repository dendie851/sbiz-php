<?php 
	...
	if($isBundling == '1') {
		...
		while($dataSalesOrderDetailBundling = mysql_fetch_array($rstSalesOrderDetailBundling)) {
			...
			$stockBundling = ($salesOrderDetailBundlingQty * $qty);
			$query = "update stuff
				set stock = stock + '$stockBundling'
			...
			$descriptionHistory = "Pembatalan Pejualan Barang dgn No Sales Order : $noOrder, Produk Bundling $nameProduk";
			
			$query = "insert stuff_history
				set stuff_id = '$salesOrderDetailBundlingStuffId',
				  tipe = '1',
				  amount = '$stockBundling',
				  date = now(),
		...
		$query = "delete from sales_order_detail_bundling
			where sales_order_detail_id = '$salesOrderDetilId'";
		mysql_query($query) or die (mysql_error());
