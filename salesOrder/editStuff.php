		...
		<form action="editStuffSave.php" method="post">
			...
			<table width="">
				<tr>
					...
					<td>
						<?php if($data['is_bundling'] == '1'): ?>
							<input name="totalHidden" type="hidden" value="<?php echo $data['amount'] ?>" />
							<input id="total" onkeyup="isNumberKey(<?php echo ($data['amount'] + $sisaStock) ?>)" type="number" name="total"  min="1" size="10" type="text" value="<?php echo $data['amount'] ?>" /><br /><small style="font-size:9px">Jenis Produk Bundling</small>
						<?php else: ?>	
							<input name="totalHidden" type="hidden" value="<?php echo $data['amount'] ?>" />
