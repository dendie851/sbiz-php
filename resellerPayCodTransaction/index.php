...
<?php ob_start(); ?>
	<table width="100%">
		<tr>
			<td align="left"><h1>COD RESELLER BELUM DI BAYAR</h1></td>
			<td align="right">
				<input type="button" value="KEMBALI" style="width:200px" onclick='window.location="../resellerPayCod/index.php"' />
			</td>
		</tr>	
	</table>	
				...
				<table width="100%" id="tblReseller">
					<thead>			
						<tr>
							...
							<th style="font-size:12px" align="center" width="%">ONGKIR</th>													
							<th style="font-size:12px" align="center" width="%" style="font-size: 11px">NILAI TRANSAKSI</th>
							<th style="font-size:12px" align="center" width="%">TOTAL COD</th>											
							<th style="font-size:12px" align="center" width="%">LEBIH BAYAR</th>													
						</tr>	
					</thead>
					<tbody>
						<?php $i=1; ?>
						...
						<?php while($val = mysql_fetch_array($data)): ?>
							<tr style="cursor:pointer">
								...
								<td align="center"><?php echo number_format($val['shipping_cost'],0,'','.') ?></td>							
								<td align="center"><?php echo number_format($val['total_nilai'],0,'','.') ?></td>							
								<td align="center"><?php echo number_format($val['amount_reseller_to_customer'],0,'','.') ?></td>							
								<td align="center"><?php echo number_format(($val['amount_reseller_to_customer'] - $val['total_nilai']),0,'','.') ?></td>
							</tr>																
							<?php $totalShippingCost = ($totalShippingCost + $val['shipping_cost']) ?>
							<?php $totalNilaiJual = ($totalNilaiJual + $val['total_nilai']) ?>
							<?php $totalCod = ($totalCod + $val['amount_reseller_to_customer']) ?>
							<?php $totalKelebihanCod = ($totalKelebihanCod + ($val['amount_reseller_to_customer'] - $val['total_nilai'])) ?>
							<?php $i++; ?>
						<?php endwhile; ?>
					</tbody>
					<tfoot>
						<th colspan="5">TOTAL</th>
						<th><?php echo number_format($totalShippingCost,0,'','.') ?></th>
						<th><?php echo number_format($totalNilaiJual,0,'','.') ?></th>
						<th><?php echo number_format($totalCod,0,'','.') ?></th>
						<th><?php echo number_format($totalKelebihanCod,0,'','.') ?></th>
					</tfoot>	
				</table>			
			</div>
			...
			<fieldset style="margin-top: 15px">
			   <div id="modalPaymentx" style="margin:2px" style="display: block;">
				...
				<table width="100%" >
					...
					<tr>
						<td colspan="2"><input type="button" onclick="sendTransferCommision()" value="TRANSFER LEBIH BAYAR" style="width: 100%" /></td>
					</tr>	
				</table>
			   </div>		
			</fieldset>   
